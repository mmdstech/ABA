...
packages <- c("tidyr","dplyr","bayesrules", "ggplot2", "gt")
...
### Prior
To convey uncertainty about any prior knowledge, we are using a $Beta(1,1)$ PDF

$$
\begin{aligned}
  \pi &\sim \text{Beta}(1,1) \\
  \alpha_{\text{prior}} &= 1, \quad \beta_{\text{prior}} = 1
\end{aligned}
$$

Uncertainty with regards to $\pi$. Intervals of the same width are equally probable.
```{r}
#| label: beta-1-1-plot
#| fig-width: 5
#| fig-height: 3
...
\begin{aligned}
  ...
  \pi \mid (Y = 87) &\sim \text{Beta}(1 + 87, 1 + 358 - 87)
\end{aligned}
$$
...
plot_beta_binomial(alpha = 1, beta = 1, y = 87, n = 358)
```

### Beta-Binomial Summary

```{r}
summarize_beta_binomial(alpha = 1, beta = 1, y = 87, n = 358) %>%
  gt() %>%
